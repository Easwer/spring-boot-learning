...
public class SystemService extends BaseService implements SystemController {
    ...
    @Override
    public ResponseEntity<Response> getServerStatus(@RoleAccess(role = UserRoleEnum.DEFAULT) final UUID authToken) {
        return createResponse("Alive", ResponseStatus.SUCCESS, null, HttpStatus.OK);
    }

    @Override
    public ResponseEntity<Response> isValidToken(@RoleAccess(role = UserRoleEnum.DEFAULT) final UUID authToken) {
