    ...
    public ResponseEntity<Response> getGlobalSettings(final String key, final String module) {
        ...
        if (key != null) {
            ...
            if (result.isPresent()) {
                ...
                return createResponse(MessageConstants.GLOBAL_SETTINGS_INVALID_KEY,
                        ResponseStatus.FAILURE, settings, HttpStatus.OK);
            }
        } else if (key != module) {
            settings = globalSettingsRepository.findByModule(module);
            if (settings == null || settings.isEmpty()) {
                return createResponse(MessageConstants.GLOBAL_SETTINGS_INVALID_MODULE,
                        ResponseStatus.FAILURE, settings, HttpStatus.BAD_REQUEST);
            }
        } else {
        ...
        if (settings == null || settings.isEmpty()) {
            return createResponse(MessageConstants.GLOBAL_SETTINGS_NO_CONTENT,
                    ResponseStatus.SUCCESS, null, HttpStatus.NO_CONTENT);
        }

        return createResponse(MessageConstants.GLOBAL_SETTINGS_RETRIVED_SUCCESSFULLY,
                ResponseStatus.SUCCESS, settings, HttpStatus.OK);
    }
